...
Yes. OmniClaw uses Docker, which works on both macOS and Linux. Just install Docker and run `/setup`.
```
...
\`\`\`bash
docker images | grep omniclaw-agent
echo '{}' | docker run -i --entrypoint /bin/echo omniclaw-agent:latest "Container OK" || echo "Container build failed"
\`\`\`
```
...
```bash
echo '{}' | docker run -i --entrypoint /bin/echo omniclaw-agent:latest "Container OK"
```
...
docker run --rm --entrypoint /bin/bash -v /tmp/test-ro:/test:ro omniclaw-agent:latest \
...
```bash
mkdir -p /tmp/test-rw
docker run --rm --entrypoint /bin/bash -v /tmp/test-rw:/test omniclaw-agent:latest \
