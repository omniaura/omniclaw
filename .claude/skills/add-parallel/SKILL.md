...
Adds Parallel AI MCP integration to OmniClaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses OmniClaw scheduler for result polling (no container blocking)
...
2. OmniClaw already set up and running
...
Add Parallel AI MCP servers after the omniclaw server:
...
3. Create a polling scheduled task using `mcp__omniclaw__schedule_task`:
```
...
3.  Send results to user with mcp**omniclaw**send_message
4.  Use mcp**omniclaw**complete_scheduled_task to mark this task as done
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo omniclaw-agent:latest "Container OK"
```
...
- Check task runs: `tail -f logs/omniclaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.omniclaw`
