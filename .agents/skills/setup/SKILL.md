...
- **Option 3 — `sudo fdesetup authrestart` for planned reboots.** Stashes the FileVault key in EFI for the _next_ boot only, so a user-initiated reboot (e.g. macOS update) comes back fully unattended. Does not help with surprise panics — Apple deliberately closes that path for security reasons.
