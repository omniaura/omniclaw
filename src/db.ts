...
export function getNewMessages(
  ...
  const rows = db
    .prepare(sql)
    .all(lastTimestamp, ...jids, `${botPrefix}:%`) as Array<NewMessage & { mentions: string | null; sender_user_id: string | null }>;
...
export function getMessagesSince(
  ...
  const rows = db
    .prepare(sql)
    .all(chatJid, sinceTimestamp, `${botPrefix}:%`) as Array<NewMessage & { mentions: string | null; sender_user_id: string | null }>;
  return rows.map((row) => ({
    ...row,
    sender_user_id: row.sender_user_id ?? undefined,
    mentions: row.mentions ? JSON.parse(row.mentions) : undefined,
