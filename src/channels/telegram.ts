  ...
  async connect(): Promise<void> {
    ...
    this.bot.on('message:text', async (ctx) => {
      ...
      if (!senderName || senderName === 'Unknown') {
        logger.warn(
          { op: 'senderIdentity', counter: 'sender_name_empty', platform: 'telegram', sender },
          'Telegram message has empty/unknown sender_name',
        );
      } else if (senderName === sender) {
        logger.warn(
          { op: 'senderIdentity', counter: 'sender_name_fallback_to_id', platform: 'telegram', sender, sender_name: senderName },
