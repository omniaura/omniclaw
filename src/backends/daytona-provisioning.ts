    ...
    logger.warn(
      {
        ...
        output: result.result?.slice(-500),
      },
      `Provisioning step failed: ${name}`,
    );
  }
 ...
 */
export async function isDaytonaProvisioned(
  sandbox: SandboxHandle,
): Promise<boolean> {
  try {
    const workdir = (await sandbox.getWorkDir()) || '/home/daytona';
    const result = await sandbox.process.executeCommand(
      `test -f ${workdir}/workspace/.nanoclaw-provisioned`,
    );
...
export async function provisionDaytona(
  ...
  if (await isDaytonaProvisioned(sandbox)) {
    logger.info(
      { sandbox: sandboxName },
      'Daytona sandbox already provisioned, skipping',
    );
    return;
  }
  ...
  logger.info(
    { sandbox: sandboxName },
    'Provisioning Daytona sandbox (first-time setup)...',
  );
  ...
  if (ghCheck.exitCode === 0) {
    ...
    logger.info(
      { sandbox: sandboxName, version: verify.result?.trim() },
      'gh CLI installed',
    );
  }
  ...
  logger.info(
    { sandbox: sandboxName, workdir, homedir },
    'Daytona sandbox provisioning complete',
  );
}
