...
function conversationsScript(): string {
  return [
    ...
    'if(!chatList)return;',
    'var currentJid=null;var messageCache={};var PAGE_SIZE=100;var searchMode=false;var searchTimer=null;var currentSearchToken=0;',
    '',
    // Tab switching
    ...
    '  fetch("/api/messages/search?q="+encodeURIComponent(trimmed)+"&limit=50")',
    '  .then(function(r){if(!r.ok)throw new Error("Failed");return r.json();})',
    ...
    '    var r=results[i];var sn=r.sender_name||r.sender||"Unknown";',
...
function agentDetailScript(): string {
  return [
    ...
    '',
    'var labels={executing:"executing","running-task":"task",idle:"idle",queued:"queued",offline:"offline"};',
    'var css={executing:"exec-executing","running-task":"exec-task",idle:"exec-idle",queued:"exec-queued",offline:"exec-offline"};',
    '',
    ...
    'function update(){',
    '  fetch("/api/ipc/queue").then(function(r){return r.json();}).then(function(details){',
    '    var s=deriveStatus(details);',
    '    statusEl.textContent=labels[s]||s;',
    '    statusEl.className="badge badge-sm "+css[s];',
    '  }).catch(function(){});',
    '}',
    '',
    'update();',
