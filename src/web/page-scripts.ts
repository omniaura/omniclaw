...
function networkScript(): string {
  return [
    'var pollTimer=null;',
    '',
    'function networkButtonAction(button){',
    '  if(!button)return;',
    '  var action=button.dataset.action||"";',
    '  if(action==="close-sync"){closeSyncPanel();return;}',
    '  if(action==="refresh-sync"){openSyncPanel(button.dataset.instanceId||"");return;}',
    '  if(action==="bulk-sync"){bulkSync(button.dataset.direction||"",button.dataset.instanceId||"");return;}',
    '  if(action==="sync-file"){',
    '    syncFile(button.dataset.direction||"",(button.dataset.instanceId||"")+"|"+(button.dataset.path||""));',
    '    return;',
    '  }',
    '  networkAction(action,button.dataset.id||button.dataset.instanceId||button.dataset.requestId||"");',
    '}',
    'window.networkButtonAction=networkButtonAction;',
    '',
    'function renderPeerStatusBadge(peer){',
    '  switch(peer.status){',
    '    case "discovered": return "<span class=\\"badge\\">discovered</span>";',
    '    case "trusted": return "<span class=\\"badge badge-admin\\">trusted</span>";',
    '    case "pending": return "<span class=\\"badge\\" style=\\"background:var(--warning);color:#000\\">pending</span>";',
    '    case "revoked": return "<span class=\\"badge\\" style=\\"background:var(--red);color:#fff\\">revoked</span>";',
    ...
    '    return "<button class=\\"btn btn-sm\\" data-action=\\"browse\\" data-instance-id=\\""+window.__esc(peer.instanceId)+"\\" onclick=\\"networkButtonAction(this)\\">Browse</button> "',
    '      +"<button class=\\"btn btn-sm btn-primary\\" data-action=\\"sync\\" data-instance-id=\\""+window.__esc(peer.instanceId)+"\\" onclick=\\"networkButtonAction(this)\\">Sync</button> "',
    '      +"<button class=\\"btn btn-sm btn-danger\\" data-action=\\"revoke\\" data-instance-id=\\""+window.__esc(peer.instanceId)+"\\" onclick=\\"networkButtonAction(this)\\">Revoke</button>";',
    '  }',
    '  if(peer.status==="pending"){',
    '    return "<button class=\\"btn btn-sm\\" data-action=\\"request\\" data-instance-id=\\""+window.__esc(peer.instanceId)+"\\" onclick=\\"networkButtonAction(this)\\">Check Approval</button>";',
    '  }',
    '  if(peer.online){',
    '    return "<button class=\\"btn btn-sm btn-primary\\" data-action=\\"request\\" data-instance-id=\\""+window.__esc(peer.instanceId)+"\\" onclick=\\"networkButtonAction(this)\\">Request Access</button>";',
    '  }',
    '  return "<span style=\\"color:var(--text-muted);font-size:0.8rem\\">offline</span>";',
    '}',
    '',
    'function renderPeersTable(peers){',
    '  var container=document.getElementById("peers-container");if(!container)return;',
    '  if(!Array.isArray(peers)||peers.length===0){',
    '    container.innerHTML="<div style=\\"padding:2rem;text-align:center;color:var(--text-muted)\\">No peers discovered yet. Ensure DISCOVERY_ENABLED=true on all instances.</div>";',
    '    return;',
    '  }',
    '  var rows=peers.map(function(peer){',
    '    var onlineDot=peer.online?"<span style=\\"color:var(--green)\\">●</span>":"<span style=\\"color:var(--text-muted)\\">○</span>";',
    '    return "<tr data-instance-id=\\""+window.__esc(peer.instanceId)+"\\">"',
    '      +"<td><strong>"+window.__esc(peer.name)+"</strong></td>"',
    '      +"<td><code>"+window.__esc(peer.host)+":"+peer.port+"</code></td>"',
    '      +"<td>"+renderPeerStatusBadge(peer)+"</td>"',
    '      +"<td>"+onlineDot+"</td>"',
    '      +"<td>"+renderPeerActions(peer)+"</td>"',
    '      +"</tr>";',
    '  }).join("");',
    '  container.innerHTML="<table class=\\"data-table\\"><thead><tr><th>name</th><th>address</th><th>trust</th><th>online</th><th>actions</th></tr></thead><tbody id=\\"peers-tbody\\">"+rows+"</tbody></table>";',
    '}',
    '',
    ...
    '    var el=document.getElementById("pending-requests");if(!el)return;',
    '    if(reqs.length===0){',
    '      el.innerHTML=\'<div style="padding:1.5rem;text-align:center;color:var(--text-muted);font-size:0.85rem">No pending requests</div>\';',
    '      return;',
    '    }',
    ...
    "        +'<code>'+window.__esc(r.fromHost)+':'+r.fromPort+'</code></div>'",
    '        +\'<div style="display:flex;gap:0.5rem">\'',
    '        +\'<button class="btn btn-sm btn-primary" data-action="approve" data-request-id="\'+window.__esc(r.id)+\'" onclick="networkButtonAction(this)">Approve</button>\'',
    '        +\'<button class="btn btn-sm btn-danger" data-action="reject" data-request-id="\'+window.__esc(r.id)+\'" onclick="networkButtonAction(this)">Reject</button>\'',
    "        +'</div></div>';",
    '    }).join("");',
    '  }).catch(function(){});',
    '}',
    ...
    '    +\'<div style="display:flex;gap:0.5rem">\'',
    '    +\'<button class="btn btn-sm" data-action="close-sync" onclick="networkButtonAction(this)">Close</button>\'',
    '    +\'<button class="btn btn-sm" data-action="refresh-sync" data-instance-id="\'+window.__esc(instanceId)+\'" onclick="networkButtonAction(this)">Refresh</button>\'',
    "    +'</div></div>';",
    '',
    '  // Stats bar',
    ...
    "      +'<th>path</th><th>status</th><th>local size</th><th>remote size</th><th>actions</th>'",
    "      +'</tr></thead><tbody>';",
    '',
    '    // Identical files',
    ...
    "        +'<td>'+fmtBytes(d.local.size)+'</td>'",
    "        +'<td>'+fmtBytes(d.remote.size)+'</td>'",
    '        +\'<td><button class="btn btn-sm" data-action="sync-file" data-direction="push" data-instance-id="\'+window.__esc(instanceId)+\'" data-path="\'+window.__esc(d.local.path)+\'" onclick="networkButtonAction(this)">Push \\u2192</button> \'',
    '        +\'<button class="btn btn-sm" data-action="sync-file" data-direction="pull" data-instance-id="\'+window.__esc(instanceId)+\'" data-path="\'+window.__esc(d.local.path)+\'" onclick="networkButtonAction(this)">\\u2190 Pull</button></td></tr>\';',
    '    });',
    '',
    '    // Local only',
    ...
    "        +'<td>'+fmtBytes(f.size)+'</td>'",
    "        +'<td>\\u2014</td>'",
    '        +\'<td><button class="btn btn-sm" data-action="sync-file" data-direction="push" data-instance-id="\'+window.__esc(instanceId)+\'" data-path="\'+window.__esc(f.path)+\'" onclick="networkButtonAction(this)">Push \\u2192</button></td></tr>\';',
    '    });',
    '',
    '    // Remote only',
    ...
    "        +'<td>\\u2014</td>'",
    "        +'<td>'+fmtBytes(f.size)+'</td>'",
    '        +\'<td><button class="btn btn-sm" data-action="sync-file" data-direction="pull" data-instance-id="\'+window.__esc(instanceId)+\'" data-path="\'+window.__esc(f.path)+\'" onclick="networkButtonAction(this)">\\u2190 Pull</button></td></tr>\';',
    '    });',
    '',
    "    h+='</tbody></table>';",
    '',
    ...
    '      if(pushable>0)h+=\'<button class="btn btn-sm btn-primary" data-action="bulk-sync" data-direction="push" data-instance-id="\'+window.__esc(instanceId)+\'" onclick="networkButtonAction(this)">Push All (\'+pushable+\') \\u2192</button>\';',
    '      if(pullable>0)h+=\'<button class="btn btn-sm" data-action="bulk-sync" data-direction="pull" data-instance-id="\'+window.__esc(instanceId)+\'" onclick="networkButtonAction(this)">\\u2190 Pull All (\'+pullable+\')</button>\';',
    "      h+='</div>';",
    '    }',
    '  }',
    '',
