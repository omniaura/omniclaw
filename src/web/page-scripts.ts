...
function ipcScript(): string {
  return [
    ...
    '      var leCell=le?("<a href=\\"/logs\\" class=\\"last-error-link\\" title=\\""+window.__esc(le.message||"")+"\\">"',
...
function agentDetailScript(): string {
  return [
    ...
    '',
    '// Message composer — inject a one-off message into a channel',
    'function onAgentSendSubmit(e){',
    '  var form=e.target.closest("[data-agent-send-form]");',
    '  if(!form)return;',
    '  e.preventDefault();',
    '  var agentId=form.getAttribute("data-agent-id");',
    '  var channelEl=form.querySelector("select[name=\\"channel\\"]");',
    '  var contentEl=form.querySelector("textarea[name=\\"content\\"]");',
    '  var senderEl=form.querySelector("input[name=\\"sender_name\\"]");',
    '  var btn=form.querySelector("button[type=\\"submit\\"]");',
    '  if(!agentId||!channelEl||!contentEl||!btn)return;',
    '  var content=contentEl.value.trim();',
    '  var channel=channelEl.value;',
    '  if(!content){window.__toast&&window.__toast("Message is empty","error");return;}',
    '  if(!channel){window.__toast&&window.__toast("Select a channel","error");return;}',
    '  var payload={channel:channel,content:content};',
    '  var sender=senderEl&&senderEl.value.trim();',
    '  if(sender)payload.sender_name=sender;',
    '  btn.disabled=true;btn.textContent="...";',
    '  fetch("/api/agents/"+encodeURIComponent(agentId)+"/message",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)})',
    '    .then(function(r){if(!r.ok)return r.json().then(function(j){throw new Error(j.error||("HTTP "+r.status));});return r.json();})',
    '    .then(function(){window.__toast&&window.__toast("Message sent");contentEl.value="";btn.disabled=false;btn.textContent="send";})',
    '    .catch(function(err){window.__toast&&window.__toast(err.message||"Failed to send","error");btn.disabled=false;btn.textContent="send";});',
    '}',
    'document.addEventListener("submit",onAgentSendSubmit);',
    '',
    'window.__cleanup=function(){clearInterval(pollTimer);document.removeEventListener("click",onAgentToggleClick);document.removeEventListener("submit",onAgentModelSubmit);document.removeEventListener("submit",onAgentSendSubmit);};',
    '})();',
  ].join('\n');
}
