...
function systemScript(): string {
  return [
    ...
    '      el=document.getElementById("sys-queue-last-error");',
    '      if(el){',
    '        var le=h.queue.last_error;',
    '        if(le){',
    '          el.textContent="";',
    '          var lea=document.createElement("a");lea.href="/logs";lea.className="last-error-link";lea.title=le.message;',
    '          var lel=document.createElement("span");lel.className="last-error-lane";lel.textContent=le.lane+" · "+le.folder;',
    '          var let_=document.createElement("span");let_.className="last-error-text";let_.textContent=le.message;',
    '          var leg=document.createElement("span");leg.className="last-error-age";leg.textContent=fmtDur(Math.max(0,Date.now()-le.at));',
    '          lea.appendChild(lel);lea.appendChild(let_);lea.appendChild(leg);el.appendChild(lea);',
    '        }else{el.textContent="—";}',
    '      }',
